<x-app-layout>
    <div class="card mb-4">
        ...
        <div class="card-body">
            <form action="{{ route('assets.update', $asset) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="col-md-6">
                        ...
                        <select id="parent_select" name="parent_category_id" class="form-select" required onchange="loadSubcategories()">
                            ...
                            @foreach($categories->where('parent_category_id', null) as $category)
                                <option value="{{ $category->id }}" {{ (old('parent_category_id') ?: ($asset->category && $asset->category->parent_category_id ? $asset->category->parent_category_id : $asset->category_id)) == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                            @endforeach
                        </select>
                ...
                <input type="hidden" id="category_id" name="category_id" value="{{ old('category_id', $asset->category_id) }}" />
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <select id="status" name="status" class="form-select" required>
                            ...
                            <option value="deployed" {{ old('status', $asset->status) == 'deployed' ? 'selected' : '' }}>Terpakai</option>
                            <option value="maintenance" {{ old('status', $asset->status) == 'maintenance' ? 'selected' : '' }}>Perawatan</option>
                            <option value="broken" {{ old('status', $asset->status) == 'broken' ? 'selected' : '' }}>Rusak</option>
                        </select>
                        @error('status') <div class="text-danger small">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-md-6">
                        <label class="form-label" for="useful_life">Masa Manfaat (Tahun)</label>
                        <input type="number" class="form-control" id="useful_life" name="useful_life"
                            value="{{ old('useful_life', $asset->useful_life) }}" min="1" max="100" required />
    ...
    <script>
        ...
        const currentCategoryId = {{ $asset->category_id }};
        ...
        function loadSubcategories() {
            ...
            if (parentId && categoryData[parentId]) {
                categoryData[parentId].forEach(sub => {
                    ...
                    option.selected = (sub.id === currentCategoryId);
        ...
        // Get existing unit identifiers from asset
        const existingUnits = @json($asset->units->pluck('unique_identifier')->values() ?? []);
        ...
        function renderUnitFields() {
            ...
            if (qty > 0) {
                ...
                for (let i = 1; i <= qty; i++) {
                    ...
                    // Populate with existing unit identifiers if available
                    if (existingUnits && existingUnits[i - 1]) {
                        input.value = existingUnits[i - 1];
                    }
